...
namespace Demo.Tests
{
    public abstract class DemoTestBase : AbpIntegratedTestBase<DemoTestModule>
    {
        ...
        protected Task<T> UsingDbContextAsync<T>(Func<DemoDbContext, Task<T>> func)
        {
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DemoDbContext, Task<T>> func)
        {
            T result;
